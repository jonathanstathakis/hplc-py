...
class IPBounds(pa.DataFrameModel):
...
class PWdwdTime(pa.DataFrameModel):
    """
...
class MapWindowsMixin:
    ...
    def _combine_intvls(
        ...
        for i, l in w_idxs.items():
            if len(l) == 1:
                w_intvls[i] = intvls[l[0]]
            elif len(l) > 1:
                left_bounds = intvls[l].apply(lambda x: x.left)
                right_bounds = intvls[l].apply(lambda x: x.right)
    ...
    def _set_peak_windows(
        ...
        intvl_df = DataFrame[PeakWindows](pd.concat(intvl_times))
    ...
    def _set_peak_wndwd_time(
        ...
        if time.dtype!=np.float64:
            raise TypeError("expected time to be NDArray of float")
    ...
    def _get_na_labels(
        ...
        labels, num_features = label(pwdwd_time[w_idx_col].isna())
    ...
    def _label_interpeaks(
        ...
        pwdwd_time['window_idx']=pwdwd_time['window_idx'].mask(pwdwd_time['window_idx'].isna(), labels-1)
        
        pwdwd_time['window_type']=pwdwd_time['window_type'].mask(pwdwd_time['window_type'].isna(), 'interpeak')
...
class MapWindows(MapWindowsMixin):
    def map_windows(
        ...
        left_bases = left_bases.astype(pd.Int64Dtype())
        right_bases = right_bases.astype(pd.Int64Dtype())
        
        intvls = self._interval_factory(left_bases, right_bases)

        w_idxs: dict[int, list[int]] = self._label_windows(intvls)
        w_intvls: dict[int, pd.Interval] = self._combine_intvls(intvls, w_idxs)
        pwdws: DataFrame[PeakWindows] = self._set_peak_windows(w_intvls, time)
        pwdwd_time: DataFrame[PWdwdTime] = self._set_peak_wndwd_time(time, pwdws)
        wdwd_time: DataFrame[WindowedTime] = self._label_interpeaks(pwdwd_time, self.pwdt_sc.window_idx)
