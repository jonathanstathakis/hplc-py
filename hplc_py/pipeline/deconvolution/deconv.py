...
import polars as pl
from hplc_py.deconvolution import definitions as dc_defs, schemas as dc_schs
...
def prepare_peak_msnts_windowed(
    ...
    whh = (split_msnt_widths
           .filter(pl.col(mp_defs.LOC) == mp_defs.KEY_WHH)
           .select(
               pl.col(mp_defs.P_IDX),
    ...
    peak_msnts = (
        pl.concat([maxima, whh])
        .to_pandas()
        .pipe(dc_schs.PeakMsnts.validate, lazy=True)
    )
...
.filter(pl.col(mp_defs.DIM) == mp_defs.X_IDX)
        ...
        .join(
            X_w.pipe(pl.from_pandas).select(
                [dc_defs.W_TYPE, dc_defs.W_IDX, dc_defs.X_IDX]
            ),
            how="left",
