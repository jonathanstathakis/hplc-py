...
for i, sig in enumerate(scales):
    for j, skew in enumerate(skews):
        ...
        _peaks = pd.DataFrame(
            np.array(
                [locs, _areas, _scales, _skews, _amps, np.arange(len(locs)) + 1]
            ).T,
            columns=[
                "retention_time",
                "area",
        ...
        _peaks["iter"] = _iter
        _peaks["peak_idx"] = np.int(_peaks["peak_idx"])
...
peak_df = pd.DataFrame(
    np.array([[10, 50], [1, 3], [0, 0], [100, 10], [sig1.sum(), sig2.sum()], [1, 2]]).T,
    columns=["retention_time", "scale", "skew", "amplitude", "area", "peak_idx"],
)
...
df = pd.DataFrame(np.array([x, sig]).T, columns=["x", "y"])
peak_df = pd.DataFrame(
    np.array([[10, 25], [1, 3], [0, 0], [100, 10], [sig1.sum(), sig2.sum()], [1, 2]]).T,
    columns=["retention_time", "scale", "skew", "amplitude", "area", "peak_idx"],
)
...
sig1 = sig1_props[0] * scipy.stats.skewnorm(
    sig1_props[1], loc=sig1_props[2], scale=sig1_props[3]
).pdf(x)
...
for i, a in enumerate(amps):
    ...
    _df = pd.DataFrame(
        ...
        columns=["amplitude", "skew", "retention_time", "scale", "area", "peak_id"],
    )
    _df["iter"] = i + 1
...
for i, w in enumerate(windows):
    area = chrom_df[(chrom_df["time"] >= w[0]) & (chrom_df["time"] <= w[1])][
        "signal"
    ].sum()
    _df = pd.DataFrame(
