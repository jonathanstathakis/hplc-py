...
def test_bg_estimation():
    ...
    assert np.isclose(chrom._signal_df['estimated_background'].values[window:-window],
...
def test_crop():
    ...
    chrom.crop([10, 20])
    assert (chrom._signal_df.x.values[0] >= 10) & (chrom._signal_df.x.values[-1] <= 20)
    _ = chrom.fit_peaks()
    try:
        chrom.crop([1, 2])
        assert False
...
def test_generic_param_bounding():
    ...
    _loc = chrom._signal_df['time'].values[chrom._peak_indices]
    _amp = chrom._signal_df['signal_corrected'].values[chrom._peak_indices]
